...
const App: React.FC = () => {
  ...
  const [currentBaseModel, setCurrentBaseModel] = useState<string>('RunawayDevil');
  ...
  useEffect(() => {
    ...
    // Initial Model Load
    engine.loadInitialModel(Generators.RunawayDevil());
  ...
  const handleRebuild = (type: 'RunawayDevil' | 'Cat' | 'Rabbit' | 'TwinDevils') => {
    const generator = Generators[type];
    if (generator && engineRef.current) {
